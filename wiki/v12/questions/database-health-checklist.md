...
verified: false
verified_by_agent: GPT-5 2026-07-21T13:34:54Z
---
...
2. Use a superuser for the complete cluster-wide capture on this exact pin. A
   role with `pg_monitor` can read most settings and statistics, but the
   progress functions in this checkout apply a stricter role-ownership test.
   Confirm that another user's activity is visible before trusting an empty or
   null result.
...
|---|---|---|
| Complete cross-role monitoring | Use a superuser for a complete capture on this exact PostgreSQL 12 pin. Without the predefined read roles, ordinary activity and `pg_stat_statements` fields are redacted and superuser-only `pg_settings` rows are filtered. `pg_monitor` inherits `pg_read_all_settings` and `pg_read_all_stats`, removing those ordinary restrictions; however, `pg_stat_get_progress_info()` still uses role ownership instead of `pg_read_all_stats`. [user-manag.sgml#predefined-monitoring-roles](../../../raw/postgres-12/doc/src/sgml/user-manag.sgml#L524-L538) [pgstatfuncs.c#activity-access](../../../raw/postgres-12/src/backend/utils/adt/pgstatfuncs.c#L625-L655) [guc.c#pg_settings-access](../../../raw/postgres-12/src/backend/utils/misc/guc.c#L8998-L9008) [pgstatfuncs.c#progress-access](../../../raw/postgres-12/src/backend/utils/adt/pgstatfuncs.c#L490-L531) | Role membership controls access; it is not a restart or reload setting. Re-run the capture after changing the monitoring role. |
...
Absence is not proof that a signal is healthy. A superuser-only setting can be
filtered from `pg_settings`; activity and extension fields can be null because
of access checks; and an event-specific log can be absent simply because no
qualifying event occurred during the interval. Resolve configuration and
access first.
...
- [ ] On a standby, capture recovery-conflict deltas by reason. The view records
  ...
  These counters are not application `lock_timeout` events or the primary
  `pg_stat_database.deadlocks` counter. The view contains useful data only on
  standby servers.
  [monitoring.sgml#pg_stat_database_conflicts](../../../raw/postgres-12/doc/src/sgml/monitoring.sgml#L2640-L2702)
  [standby.c#recovery-conflict-reasons](../../../raw/postgres-12/src/backend/storage/ipc/standby.c#L371-L418)
...
is overdue.
[autovacuum.c#relation_needs_vacanalyze](../../../raw/postgres-12/src/backend/postmaster/autovacuum.c#L2920-L2955)
[autovacuum.c#effective-thresholds](../../../raw/postgres-12/src/backend/postmaster/autovacuum.c#L2995-L3080)
...
[autovacuum.c#table-and-TOAST-passes](../../../raw/postgres-12/src/backend/postmaster/autovacuum.c#L2035-L2067)
[analyze.c#do_analyze_rel](../../../raw/postgres-12/src/backend/commands/analyze.c#L231-L259)
...
- [ ] Treat `buffers_backend_fsync` as the stronger signal. PostgreSQL
  ...
  compaction.
  [checkpointer.c#ForwardFsyncRequest](../../../raw/postgres-12/src/backend/postmaster/checkpointer.c#L1086-L1171)
...
- [ ] Investigate an old `backend_xmin` on a sender. It can expose a retention
  ...
  [high-availability.sgml#hot_standby_feedback](../../../raw/postgres-12/doc/src/sgml/high-availability.sgml#L895-L903)
...
[initdb.c#system_views-input](../../../raw/postgres-12/src/bin/initdb/initdb.c#L2612-L2624)
...
| Vacuum, analyze, TOAST, partitions, and wraparound | [autovacuum.c#table-and-TOAST-passes](../../../raw/postgres-12/src/backend/postmaster/autovacuum.c#L2035-L2067), [autovacuum.c#relation_needs_vacanalyze](../../../raw/postgres-12/src/backend/postmaster/autovacuum.c#L2925-L3080), [analyze.c#do_analyze_rel](../../../raw/postgres-12/src/backend/commands/analyze.c#L231-L259), [multixact.c#MultiXactMemberFreezeThreshold](../../../raw/postgres-12/src/backend/access/transam/multixact.c#L2785-L2844) |
| Checkpoints and background writes | [bufmgr.c#BgBufferSync](../../../raw/postgres-12/src/backend/storage/buffer/bufmgr.c#L2260-L2300), [checkpointer.c#ForwardFsyncRequest](../../../raw/postgres-12/src/backend/postmaster/checkpointer.c#L1086-L1171), [checkpointer.c#checkpoint-warning](../../../raw/postgres-12/src/backend/postmaster/checkpointer.c#L447-L462) |
...
| Tests and build/generated boundary | [stats.sql#wait_for_stats](../../../raw/postgres-12/src/test/regress/sql/stats.sql#L17-L77), [prepared_xacts.sql#view-and-lock-tests](../../../raw/postgres-12/src/test/regress/sql/prepared_xacts.sql#L110-L153), [catalog/Makefile#catalog-generation](../../../raw/postgres-12/src/backend/catalog/Makefile#L51-L109), [initdb.c#system_views-input](../../../raw/postgres-12/src/bin/initdb/initdb.c#L2612-L2624) |
...
- The predefined-role documentation describes `pg_read_all_stats` as reading
  ...
  checks role ownership instead. This page follows the implementation and
  requires superuser for complete cross-role progress.
