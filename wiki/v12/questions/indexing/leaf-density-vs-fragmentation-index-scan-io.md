...
- Exact-pin execution: one isolated PostgreSQL 12.2 server built from `45b88269a353ad93744772791feb6d01bc7e1e42` with `pgstattuple` and `pageinspect` installed, used for the density, fragmentation, and dead-space measurements and for validating the survey query. The server was stopped after testing. Its data directory and SQL scripts were deleted on 2026-09-07, so nothing under `.wiki-runtime/` reproduces these numbers; the closely related fixtures are published under [Reproduction](leaf-density-60-vs-90-query-impact.md#reproduction) on the leaf-density question page.
...
- Two figures shared with the leaf-density question page did not reproduce. A 2026-09-07 rebuild of the same fixture at the same pin read 2736 and 4119 warm index-only scan buffers rather than 2738 and 4121, and default parallel plan totals of 22188.97 and 27740.97 rather than 22647.09 and 28199.09; every other value in that fixture, including the leaf-page counts, densities, `relpages` and the serial costs, reproduced exactly. The re-measurement also showed that one of those buffers belongs to the heap relation's visibility map rather than to the index, so the buffer columns above are plan-node totals, not index-only totals. The original fixture SQL had already been deleted, so the difference cannot be traced. See [Open Questions](leaf-density-60-vs-90-query-impact.md#open-questions) on the leaf-density question page; the tables on this page have not been re-run as a whole.
