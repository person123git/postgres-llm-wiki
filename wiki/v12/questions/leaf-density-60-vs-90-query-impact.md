...
- Over time, inserts, updates that move rows, and page splits that do not perfectly repack cause the average to drop. Deletes create LP_DEAD items that VACUUM can remove, but the reclaimed space is not always reused on the original page.
...
- v12 `EXPLAIN (ANALYZE, BUFFERS)` prints buffer counters from each plan node's `Instrumentation.bufusage`, and `show_buffer_usage` labels them only as shared/local/temp hit/read/dirtied/written plus optional I/O timing. It does not split a node's buffers into index-relation versus heap-relation pages ([explain.c#plan-buffer-usage](../../../raw/postgres-12/src/backend/commands/explain.c#L1864-L1866), [explain.c#show_buffer_usage](../../../raw/postgres-12/src/backend/commands/explain.c#L2867-L2978)).
...
| `btvacuumcleanup` / `btvacuumscan` supply index tuple and page counts that VACUUM can write to `pg_class` | [nbtree.c#btvacuumcleanup](../../../raw/postgres-12/src/backend/access/nbtree/nbtree.c#L897-L940), [nbtree.c#btvacuumscan-stats](../../../raw/postgres-12/src/backend/access/nbtree/nbtree.c#L1015-L1094), [vacuumlazy.c#lazy_cleanup_index](../../../raw/postgres-12/src/backend/access/heap/vacuumlazy.c#L1772-L1817) |
| EXPLAIN BUFFERS reports node-level shared/local/temp counters, not relation-kind counters | [explain.c#plan-buffer-usage](../../../raw/postgres-12/src/backend/commands/explain.c#L1864-L1866), [explain.c#show_buffer_usage](../../../raw/postgres-12/src/backend/commands/explain.c#L2867-L2978) |
...
- PostgreSQL 12 B-tree sources: `nbtree.c`, `nbtsearch.c`, `nbtpage.c`, `nbtinsert.c`, `nbtsplitloc.c`, `nbtsort.c`, and `nbtree.h`.
...
- [nbtsearch.c#_bt_readnextpage and _bt_walk_left](../../../raw/postgres-12/src/backend/access/nbtree/nbtsearch.c#L1726-L2007)
...
- [nbtree.c#btgettuple, btgetbitmap, btvacuumcleanup](../../../raw/postgres-12/src/backend/access/nbtree/nbtree.c#L215-L330, L897-L996)
