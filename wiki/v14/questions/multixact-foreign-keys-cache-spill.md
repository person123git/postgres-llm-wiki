...
verified: false
verified_by_agent: GPT-5-6-Sol-Max-Thinking 2026-07-13T20:08:07Z
---
...
- `GetMultiXactIdMembers` reads an existing definition and may add it to only the calling backend's local cache; that read does not allocate another MXID [multixact.c#GetMembers](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1333-L1585).
...
- the MXID counter refuses commands that need a new MXID within three million IDs of its wrap limit [multixact.c#MXID-stop](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1138-L1165) [multixact.c#SetMultiXactIdLimit](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L2362-L2405);
- the member counter raises `multixact "members" limit exceeded` when the requested member range would cross `offsetStopLimit` [multixact.c#member-stop](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1228-L1264).

Both stop only commands that need a new MXID, not every write [maintenance.sgml#MXID-exhaustion-boundary](../../../raw/postgres-14/doc/src/sgml/maintenance.sgml#L793-L811).
...
|---|---|---|---|---|
| `vacuum_multixact_freeze_min_age` | `PGC_USERSET` | session/transaction; configuration-default changes take reload | 5,000,000 | Replacement cutoff while VACUUM scans [guc.c#vacuum_multixact_freeze_min_age](../../../raw/postgres-14/src/backend/utils/misc/guc.c#L2636-L2644) |
| `vacuum_multixact_freeze_table_age` | `PGC_USERSET` | session/transaction; configuration-default changes take reload | 150,000,000 | Age that makes VACUUM aggressive [guc.c#vacuum_multixact_freeze_table_age](../../../raw/postgres-14/src/backend/utils/misc/guc.c#L2646-L2654) |
| `autovacuum_multixact_freeze_max_age` | `PGC_POSTMASTER` | **server restart** | 400,000,000 | Forces anti-wraparound autovacuum and caps the two ages above [guc.c#autovacuum_multixact_freeze_max_age](../../../raw/postgres-14/src/backend/utils/misc/guc.c#L3291-L3300) |
| `vacuum_multixact_failsafe_age` | `PGC_USERSET` | session/transaction; configuration-default changes take reload | 1,600,000,000 | During a VACUUM, disables cost delay and bypasses nonessential work after the failsafe threshold [guc.c#vacuum_multixact_failsafe_age](../../../raw/postgres-14/src/backend/utils/misc/guc.c#L2674-L2681) [config.sgml#vacuum_multixact_failsafe_age](../../../raw/postgres-14/doc/src/sgml/config.sgml#L8847-L8876) |
...
The exact-pin isolated build's full `src/test/isolation` check passed during this review, including all tests above. The test schedule contains direct correctness tests, but no test references `MAX_CACHE_ENTRIES`, the local-cache functions, `MultiXactOffset`/`MultiXactMember` SLRU statistics, or `SLRURead`; the checkout therefore has no direct cache-eviction, SLRU-thrash, or performance benchmark [isolation_schedule](../../../raw/postgres-14/src/test/isolation/isolation_schedule#L30-L75) [multixact.c#MAX_CACHE_ENTRIES](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L313-L324).
...
- Isolation specs and expected outputs for `fk-contention`, `multixact-no-deadlock`, `multixact-no-forget`, `tuplelock-conflict`, `nowait-2`, `skip-locked-2`, and `freeze-the-dead`; the full exact-pin isolation suite passed in `.wiki-runtime/`.
- The three filed diagnostic queries and both tagged timeout statements executed successfully against the isolated exact-pin PostgreSQL 14 server; the server was then stopped.
...
| Two shared SLRUs use 8 offset and 16 member pages under `pg_multixact/` | [multixact.h#SLRU-buffers](../../../raw/postgres-14/src/include/access/multixact.h#L30-L34), [multixact.c#MultiXactShmemInit](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1950-L2015) |
| WAL/checkpoint/recovery rebuild and persist MultiXact state | [multixact.c#WAL-design](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L27-L49), [multixact.c#CheckPointMultiXact](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L2288-L2305), [multixact.c#multixact_redo](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L3393-L3521) |
...
| Cache is backend-local, transaction-local, LRU, and capped at 256 entries | [multixact.c#cache-definition](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L296-L324), [multixact.c#cache-lookups](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1612-L1707), [multixact.c#mXactCachePut](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1710-L1758) |
| Eviction frees memory and does not write/spill the entry | [multixact.c#mXactCachePut-eviction](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1742-L1757) |
| Cache miss first reaches shared SLRU; only an SLRU miss requests a file read | [multixact.c#GetMultiXactIdMembers](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1386-L1585), [slru.c#SimpleLruReadPage](../../../raw/postgres-14/src/backend/access/transam/slru.c#L394-L476) |
...
| Creation traffic and one backend's cache pressure are distinct | [multixact.c#CreateFromMembers](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L789-L861), [multixact.c#GetMembers](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1333-L1585) |
...
| MXID reservation uses `MultiXactGenLock`; full member arrays enter WAL/SLRU | [multixact.c#GetNewMultiXactId](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1075-L1329), [multixact.c#RecordNewMultiXact](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L869-L1071) |
| Member reads use exclusive SLRU control locks but release them for I/O | [multixact.c#GetMembers-exclusive-locks](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1472-L1572), [slru.c#SimpleLruReadPage-I/O](../../../raw/postgres-14/src/backend/access/transam/slru.c#L434-L476) |
| Member pressure lowers effective freeze age and makes VACUUM more aggressive | [multixact.c#MultiXactMemberFreezeThreshold](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L2933-L3000), [maintenance.sgml#aggressive-MXID-vacuum](../../../raw/postgres-14/doc/src/sgml/maintenance.sgml#L750-L783) |
| MXID-counter and member-counter stops are separate | [multixact.c#MXID-stop](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1138-L1165), [multixact.c#member-stop](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1228-L1264) |
...
| Four direct GUCs and three per-table reloptions have the stated scopes | [guc.c#MultiXact-vacuum-settings](../../../raw/postgres-14/src/backend/utils/misc/guc.c#L2636-L2681), [guc.c#autovacuum_multixact_freeze_max_age](../../../raw/postgres-14/src/backend/utils/misc/guc.c#L3291-L3300), [reloptions.c#MultiXact-reloptions](../../../raw/postgres-14/src/backend/access/common/reloptions.c#L263-L305) |
