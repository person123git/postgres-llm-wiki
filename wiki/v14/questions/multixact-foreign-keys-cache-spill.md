...
- **Explicit row locks**: `SELECT ... FOR KEY SHARE / FOR SHARE / FOR NO KEY UPDATE / FOR UPDATE` on rows already locked by another live transaction. `FOR SHARE` on a popular row by many sessions is a direct MultiXact generator.
...
Resolving a MultiXactId to its members in `GetMultiXactIdMembers` holds the SLRU control lock **exclusively** for both the offsets and the members read (see the read path above), so — unlike CLOG's `SimpleLruReadPage_ReadOnly` shared-lock fast path [slru.c#ReadOnly](../../../raw/postgres-14/src/backend/access/transam/slru.c#L494-L525) — every backend resolving a MultiXact serializes on `MultiXactOffsetSLRULock` and `MultiXactMemberSLRULock` [lwlocknames.txt](../../../raw/postgres-14/src/backend/storage/lmgr/lwlocknames.txt#L21-L23). Under buffer pressure the victim search itself stalls: `SlruSelectLRUPage` must wait when every page is I/O-busy [slru.c#all-busy](../../../raw/postgres-14/src/backend/access/transam/slru.c#L1117-L1128), and if the chosen victim is dirty it must be written out first, synchronously, while the control lock is held [slru.c#write-victim](../../../raw/postgres-14/src/backend/access/transam/slru.c#L1130-L1139). Concurrent backends needing a page that is mid-read block in `SimpleLruWaitIO` [slru.c#SimpleLruWaitIO](../../../raw/postgres-14/src/backend/access/transam/slru.c#L341-L349). The visible symptom is many sessions in `LWLock` waits on the MultiXact SLRU locks (plus the per-buffer tranches `MultiXactOffsetBuffer`/`MultiXactMemberBuffer` [lwlock.c#tranches](../../../raw/postgres-14/src/backend/storage/lmgr/lwlock.c#L138-L140)) and in `SLRURead` I/O waits — throughput collapses even though CPU is idle.
...
- `src/backend/access/transam/slru.c` and its header comment — the SLRU pool model, `SimpleLruReadPage`, `SimpleLruReadPage_ReadOnly`, `SlruSelectLRUPage`, `SlruPhysicalReadPage`, `SimpleLruWaitIO`.
...
| MultiXact reads hold control lock exclusively (no ReadOnly shared fast path) | [slru.c#ReadOnly](../../../raw/postgres-14/src/backend/access/transam/slru.c#L494-L525), [multixact.c#offset-read](../../../raw/postgres-14/src/backend/access/transam/multixact.c#L1472-L1529) |
...
| Victim search stalls when all pages busy; dirty victim written under lock | [slru.c#all-busy](../../../raw/postgres-14/src/backend/access/transam/slru.c#L1117-L1128), [slru.c#write-victim](../../../raw/postgres-14/src/backend/access/transam/slru.c#L1130-L1139) |
...
- [src/backend/access/transam/slru.c](../../../raw/postgres-14/src/backend/access/transam/slru.c) — SLRU pool model, `SimpleLruReadPage`, `SimpleLruReadPage_ReadOnly`, `SlruSelectLRUPage`, `SlruPhysicalReadPage`, `SimpleLruWaitIO`.
