...
> Prompt note: the follow-up wording is a lightly grammar-corrected restatement of the original request, applied with the asker's approval per the wiki's prompt-hygiene rule.
...
Net effect: a prepared statement, PL/pgSQL statement, or held cursor over an RLS table reuses its cached plan as long as the same role runs it with the same `row_security` value and no relevant DDL fires. The RLS rewrite (deriving and combining the security quals) is done once and reused; only the planner may re-run, and only for custom plans.

#### In what scenarios does it not work

Each of these forces the RLS rewrite and/or planning to be redone:

- **The role changes.** `SET ROLE`, `SET SESSION AUTHORIZATION`, and entering a `SECURITY DEFINER` function all change `GetUserId()` [miscinit.c#GetUserId](../../../raw/postgres-14/src/backend/utils/init/miscinit.c#L477-L481) — role/authorization via `SetCurrentRoleId()` [miscinit.c#SetCurrentRoleId](../../../raw/postgres-14/src/backend/utils/init/miscinit.c#L917-L919), and a `SECURITY DEFINER` call via `SetUserIdAndSecContext()` inside `fmgr_security_definer()` [fmgr.c#fmgr_security_definer](../../../raw/postgres-14/src/backend/utils/fmgr/fmgr.c#L716-L718). Any RLS-dependent cached query tree and generic plan are then rebuilt on next use. Connection poolers that `SET ROLE` per request are the classic case.
...
| `SET ROLE`/`SET SESSION AUTHORIZATION`/`SECURITY DEFINER` change `GetUserId()` | [miscinit.c#GetUserId](../../../raw/postgres-14/src/backend/utils/init/miscinit.c#L477-L481), [miscinit.c#SetCurrentRoleId](../../../raw/postgres-14/src/backend/utils/init/miscinit.c#L917-L919), [fmgr.c#fmgr_security_definer](../../../raw/postgres-14/src/backend/utils/fmgr/fmgr.c#L716-L718) |
...
- [src/backend/tcop/postgres.c](../../../raw/postgres-14/src/backend/tcop/postgres.c), [src/backend/executor/spi.c](../../../raw/postgres-14/src/backend/executor/spi.c) — the un-cached simple-query path and one-shot SPI plans.
- [src/backend/utils/init/miscinit.c](../../../raw/postgres-14/src/backend/utils/init/miscinit.c), [src/backend/utils/fmgr/fmgr.c](../../../raw/postgres-14/src/backend/utils/fmgr/fmgr.c) — `GetUserId`/`SetCurrentRoleId` and the `SECURITY DEFINER` user-id switch that trigger RLS re-planning.
