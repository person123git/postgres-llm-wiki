...
The local `raw/postgres-18/` checkout for this page is shallow and contains only the pinned commit, so this review could not verify the listed hashes, ancestry after the v13 development stamp, or commit-message text from local history. Treat the `Commit` column as unverified provenance. The `Current v18 evidence` column is verified against the pinned PostgreSQL 18 source and shows the current code, tests, or docs that implement each RLS-related fix area.

| Commit | Area | Unverified fix description | Current v18 evidence |
|---|---|---|---|
...
- Source-history verification gap: `raw/postgres-18/` is a shallow checkout with only the pinned commit, so local `git log`/`git show` cannot verify the listed fix hashes, commit subjects, ancestry, or completeness.
...
| Current v18 evidence exists for the listed RLS-related fix areas; the commit hashes are not locally verified. | [Fixes Since PostgreSQL 12](#fixes-since-postgresql-12), plus each row's current-source evidence. |
...
- The local `raw/postgres-18/` checkout is shallow and contains only the pinned commit, so the commit hashes, commit subjects, ancestry after the v13 development stamp, and "all fixes since PostgreSQL 12" completeness are not verified from local source history. Fetching full PostgreSQL history, if requested, is needed to verify provenance and completeness.
- Minor-release first-appearance labels are not assigned here because the local checkout history has no release tags beyond the shallow pinned commit.
