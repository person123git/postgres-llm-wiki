...
- [Proposing a Sampling pgstatindex Variant for PostgreSQL 18 (unverified)](questions/pgstatindex-sample-variant-proposal.md) - Concrete design for a `pgstatindex_approx` that random-samples physical index blocks: which columns stay exact (metapage/relation size), which ratio metrics estimate directly, which page counts scale by `1/f`, plus a pros and cons section and tests to add.
