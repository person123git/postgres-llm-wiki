...
- [Can ALTER TABLE ... ATTACH PARTITION Drop Indexes in PostgreSQL 17? (unverified)](questions/attach-partition-index-drops.md) - Answers that ATTACH PARTITION never drops an index: `AttachPartitionEnsureIndexes` is strictly match-or-create (re-parent a compatible existing partition index via `IndexSetParentIndex`, else create one via `DefineIndex`), incompatible/extra indexes are kept, and the three look-alike cases (an absorbed index can no longer be dropped on its own, the manually-dropped redundant CHECK constraint, and a later cascading DROP on the parent) are not ATTACH drops; includes a "changes since PostgreSQL 12" section (the invalid-index skip `fc55c7ff` back-patched to v11+, the v17-only PK-vs-UNIQUE constraint-type match `cee8db3f`, and mechanical attmap/`DefineIndex`/`foreach_oid` refactors), all anchored to the v17 checkout's source and commit history.
