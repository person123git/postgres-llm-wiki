...
[replication-origins.sgml:23](../../../raw/postgres-17/doc/src/sgml/replication-origins.sgml#L22-L24),
[030_origin.pl:71](../../../raw/postgres-17/src/test/subscription/t/030_origin.pl#L71-L74)).
...
| Replication origin per subscription | [worker.c#run_apply_worker](../../../raw/postgres-17/src/backend/replication/logical/worker.c#L4501-L4511) | Tags all WAL the apply worker writes |
...
| `check_publications_origin` | [subscriptioncmds.c#check_publications_origin](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L2016-L2133) | WARNING for the initial-copy blind spot |
...
The default is `any`
([subscriptioncmds.c:163](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L162-L163)).
The parameter is deliberately a string, not a boolean, so future versions can
filter by specific origin names
([subscriptioncmds.c:328](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L328-L333)).

- `CREATE SUBSCRIPTION` accepts it
  ([subscriptioncmds.c:606](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L601-L606))
  and stores it in `pg_subscription.suborigin`
  ([subscriptioncmds.c:724](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L724-L725),
...
- `ALTER SUBSCRIPTION ... SET (origin = ...)` updates the same column
  ([subscriptioncmds.c:1259](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L1259-L1264)).
  ...
  restart list
  ([worker.c#maybe_reread_subscription](../../../raw/postgres-17/src/backend/replication/logical/worker.c#L3930-L3957)).
- `psql`'s `\dRs+` shows it as the `Origin` column
  ([describe.c:6592](../../../raw/postgres-17/src/bin/psql/describe.c#L6592)).
...
([worker.c#run_apply_worker](../../../raw/postgres-17/src/backend/replication/logical/worker.c#L4501-L4511)).
...
[tablesync.c:1442](../../../raw/postgres-17/src/backend/replication/logical/tablesync.c#L1442-L1444)),
...
- Row-level records request it explicitly with
  ...
  [heapam.c#heap_multi_insert](../../../raw/postgres-17/src/backend/access/heap/heapam.c#L2587-L2588),
  [heapam.c#heap_delete](../../../raw/postgres-17/src/backend/access/heap/heapam.c#L3094-L3095),
  [heapam.c#log_heap_update](../../../raw/postgres-17/src/backend/access/heap/heapam.c#L9113-L9114),
  [heapam.c#heap_finish_speculative](../../../raw/postgres-17/src/backend/access/heap/heapam.c#L6104-L6105),
  [tablecmds.c#ExecuteTruncateGuts](../../../raw/postgres-17/src/backend/commands/tablecmds.c#L2255-L2256),
...
1. The subscriber's apply worker copies `pg_subscription.suborigin` into the
   streaming options
   ([worker.c#set_stream_options](../../../raw/postgres-17/src/backend/replication/logical/worker.c#L4393-L4394))
   ...
   ([libpqwalreceiver.c#libpqrcv_startstreaming](../../../raw/postgres-17/src/backend/replication/libpqwalreceiver/libpqwalreceiver.c#L621-L624)).
...
4. Logical decoding consults the callback through `FilterByOrigin`
   ...
   [logical.c#filter_by_origin_cb_wrapper](../../../raw/postgres-17/src/backend/replication/logical/logical.c#L1220-L1243))
   at two levels:
...
recursive insertion"
([030_origin.pl:71](../../../raw/postgres-17/src/test/subscription/t/030_origin.pl#L71-L99)).
...
A ([030_origin.pl:100](../../../raw/postgres-17/src/test/subscription/t/030_origin.pl#L100-L144)).
...
[replorigin.sql:34](../../../raw/postgres-17/contrib/test_decoding/sql/replorigin.sql#L34-L37).
...
([subscriptioncmds.c#check_publications_origin](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L2016-L2133),
call sites
[subscriptioncmds.c:763](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L763)
and
[subscriptioncmds.c:921](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L921)).
...
([030_origin.pl:44](../../../raw/postgres-17/src/test/subscription/t/030_origin.pl#L44-L69)).

The docs ship a publisher-side query to list tables that might contain
non-local rows
([create_subscription.sgml:524](../../../raw/postgres-17/doc/src/sgml/ref/create_subscription.sgml#L523-L538)).
...
- **No DDL replication** — schema changes must be applied on each node
  ([logical-replication.sgml:1694](../../../raw/postgres-17/doc/src/sgml/logical-replication.sgml#L1692-L1706)).
- **No sequence replication** — serial/identity columns need disjoint ranges
  ...
  ([logical-replication.sgml:1709](../../../raw/postgres-17/doc/src/sgml/logical-replication.sgml#L1707-L1720)).
...
- [030_origin.pl](../../../raw/postgres-17/src/test/subscription/t/030_origin.pl) —
  ...
  `f0f59b658e` below).
- [subscription.sql:69](../../../raw/postgres-17/src/test/regress/sql/subscription.sql#L69-L75) —
...
`raw/postgres-17/` at `54eeefae`) that introduce or change the
...
`Stamp 17.N` commits (`d7ec59a63d` 17.0 ... `25c49f3a4a` 17.10), verified with
...
| Commit | Date | Subject |
|---|---|---|
| `f0f59b658e` | 2026-05-11 | "Fix SQL injection in logical replication origin checks." — CVE-2026-6638: `check_publications_origin` interpolated schema/relation names unquoted during `ALTER SUBSCRIPTION ... REFRESH PUBLICATION`; now quoted with `quote_literal_cstr` ([subscriptioncmds.c:2065](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L2065-L2078)); back-patched through 16 |

No related commits exist between Stamp 17.10 (`25c49f3a4a`) and the pinned
HEAD (`54eeefae`; the only replication commit there, `f2acab5348`, is COPY
progress reporting, unrelated to origins).
...
- Apply-side tagging: `run_apply_worker`, `set_stream_options`,
  ...
  [worker.c](../../../raw/postgres-17/src/backend/replication/logical/worker.c#L4501-L4511);
...
- WAL plumbing: `XLogSetRecordFlags(XLOG_INCLUDE_ORIGIN)` call sites in
  ...
  [tablecmds.c](../../../raw/postgres-17/src/backend/commands/tablecmds.c#L2255-L2256),
...
- Publisher filtering: `START_REPLICATION` option in
  [libpqwalreceiver.c](../../../raw/postgres-17/src/backend/replication/libpqwalreceiver/libpqwalreceiver.c#L621-L624),
  `pgoutput`
  ...
  callback wrapper in
  [logical.c](../../../raw/postgres-17/src/backend/replication/logical/logical.c#L1220-L1243).
...
| Claim | Evidence |
|---|---|
| `origin` accepts only `none`/`any`, default `any`, stored in `suborigin` | [subscriptioncmds.c#parse_subscription_options](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L319-L341), [subscriptioncmds.c:163](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L162-L163), [subscriptioncmds.c:724](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L724-L725), [pg_subscription.h#suborigin](../../../raw/postgres-17/src/include/catalog/pg_subscription.h#L115) |
| Apply worker sets session origin `pg_<suboid>`; tablesync uses `pg_<suboid>_<relid>` | [worker.c#run_apply_worker](../../../raw/postgres-17/src/backend/replication/logical/worker.c#L4501-L4511), [worker.c#ReplicationOriginNameForLogicalRep](../../../raw/postgres-17/src/backend/replication/logical/worker.c#L429-L443), [tablesync.c#LogicalRepSyncTableStart](../../../raw/postgres-17/src/backend/replication/logical/tablesync.c#L1405-L1414) |
...
| Subscriber sends `origin` to publisher only for servers >= 16 | [libpqwalreceiver.c#libpqrcv_startstreaming](../../../raw/postgres-17/src/backend/replication/libpqwalreceiver/libpqwalreceiver.c#L621-L624) |
...
| Loop prevention works; transitive origins filtered | [030_origin.pl:71](../../../raw/postgres-17/src/test/subscription/t/030_origin.pl#L71-L99), [030_origin.pl:100](../../../raw/postgres-17/src/test/subscription/t/030_origin.pl#L100-L144) |
| Initial COPY cannot see origins; WARNING raised; user must verify | [subscriptioncmds.c#check_publications_origin](../../../raw/postgres-17/src/backend/commands/subscriptioncmds.c#L2016-L2133), [create_subscription.sgml#Notes](../../../raw/postgres-17/doc/src/sgml/ref/create_subscription.sgml#L511-L538) |
...
| DDL and sequences not replicated | [logical-replication.sgml:1694](../../../raw/postgres-17/doc/src/sgml/logical-replication.sgml#L1692-L1720) |
| Commit attribution to 16-cycle / 17.0 / 17.5 / 17.10 | checkout git history: `git merge-base --is-ancestor` against `d31d30973a`, `5bcc7e6dc8`, `e0b82fc8e8` (16beta1), `d7ec59a63d` (17.0), `f8554dee41` (17.4), `5e2f3df49d` (17.5), `6af885119b` (17.8), `25c49f3a4a` (17.10); commit messages of `54ccfd6586`, `0ae1245e04`, `f0f59b658e` state "Backpatch-through: 16" |
