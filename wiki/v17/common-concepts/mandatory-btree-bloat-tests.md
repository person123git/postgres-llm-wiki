...
Tests 1 to 17 plus the 11b transition control, built on two 500,000-row tables whose deduplication key columns each carry 5,000 distinct values, 100 rows per key, beside one strictly increasing column for the unique control. The family asks one question in seventeen ways: can this index deduplicate, and does the method under test know it.
...
| 11b | `i_dedup_on_after` | the same keys built with `deduplicate_items = off`, then switched on with `ALTER INDEX` and no further inserts | the build-time gate reads the option as it stands at rebuild time, so the switch changes nothing about the existing file and everything about what the oracle produces ([nbtsort.c#_bt_load-deduplicate](../../../raw/postgres-17/src/backend/access/nbtree/nbtsort.c#L1147-L1152)) |
...
Test 11b is the family's off-to-on transition control, and it tests a different condition from test 11 rather than repeating it. The reloption is not in the metapage; it is a `pg_class.reloptions` entry whose stated scope is later inserts, which is why turning it on needs only `ShareUpdateExclusiveLock` ([reloptions.c#deduplicate_items](../../../raw/postgres-17/src/backend/access/common/reloptions.c#L159-L168), [reloptions.c#AlterTableGetRelOptionsLockLevel](../../../raw/postgres-17/src/backend/access/common/reloptions.c#L2110-L2141), [tablecmds.c#AT_SetRelOptions-lock](../../../raw/postgres-17/src/backend/commands/tablecmds.c#L4693-L4704)). Two independent gates read it. On insert, `_bt_findinsertloc` runs a deduplication pass only when the option is on and the index is equal-image ([nbtinsert.c#dedup-on-insert](../../../raw/postgres-17/src/backend/access/nbtree/nbtinsert.c#L2774-L2782)); at build time `_bt_load` reads whatever the option says at that moment ([nbtsort.c#_bt_load-deduplicate](../../../raw/postgres-17/src/backend/access/nbtree/nbtsort.c#L1147-L1152)). An index built with the option off and switched on without one further insert therefore keeps a non-deduplicated file whose rebuild deduplicates, so the oracle measures a large reclaimable fraction while nothing a metapage flag or an entry count can see has changed. A suite that builds only `deduplicate_items = off` indexes and never switches the option back on does not cover this condition, and a run that omits 11b must say so rather than treat test 11 as covering it.
...
This family must keep an empty population and an unknown catalog count apart, because for an index that ended up with no entries they are not the same state and `pg_class.reltuples` shows only the second. Both the build and the oracle rebuild leave such an index at `reltuples = -1` with `relpages` untouched, not at `0`; see [The oracle and the verdict bands](#the-oracle-and-the-verdict-bands). The fixtures that reach it are 113a to 113c, 115 before its load, 116, 118 before its arrivals, and 121's `nzb_k`, whose whole point is a `REINDEX` run while the table is empty. A fixture in this family asserts its population from the index itself, and a run that prints `-1` as a zero row count has recorded a different fixture from the one the suite defines.
...
|---|---|
| `deduplicate_items` on and off, including on a unique index and for the single-value strategy | [btree_index.sql#deduplicate_items](../../../raw/postgres-17/src/test/regress/sql/btree_index.sql#L185-L223) |
...
| [nbtree.h#BTGetDeduplicateItems](../../../raw/postgres-17/src/include/access/nbtree.h#L1146-L1150) | the reloption switch tests 11 and 38 turn off and test 11b turns back on, read separately by the insert path and by the build |
...
- **Deduplication.** Family 1 decides only whether an index *may* deduplicate, and 11b separates the may-it question from the did-it question: the metapage flag and the reloption are read at different times by different gates. How much a page actually saves is a property of the insert-time pass and the write history ([nbtdedup.c#_bt_dedup_pass](../../../raw/postgres-17/src/backend/access/nbtree/nbtdedup.c#L56-L70)), which the suite treats as an outcome to measure, not an input to model.
- **VACUUM.** The suite depends on `VACUUM` to convert dead entries into reclaimable pages, and deliberately withholds it in the four fixtures where entries leave an index and nothing cleans up after them: 65, 67, 113a and 113c. A separate set - 69, 106, 117 and 121 - runs the `VACUUM` and withholds the `ANALYZE` instead. Two boundaries: the suite never asserts *when* autovacuum would have run a `VACUUM`, since rule 3 simulates the analyze side only; and it does not assert that a `VACUUM` it did run removed the entries, because the bypass has conditions of its own.
...
- The four writers of `reltuples`: a build through `index_build`'s two `index_update_stats` calls, `ANALYZE` through `tupleFract`, `VACUUM` through `update_relstats_all_indexes` unless the AM marked the count estimated, and `RelationSetNewRelfilenumber`'s `-1`; plus `TRUNCATE`'s new relfilenode, `AddNewRelationTuple`'s initial `-1`, and `index_update_stats`'s hack that preserves `-1` for an empty relation.
...
- The shipped tests that touch the same behavior - `btree_index.sql`, `check_btree.sql`, `pageinspect`'s `btree.sql`, `index_including.sql`, `pgstattuple.sql`, `stats.sql`, `vacuum.sql` - and `pg_regress`'s extra configuration, which is where the absence of launcher and bypass coverage is visible.
...
| Test 11b's switch is read at build time, so it changes what a rebuild produces | [nbtsort.c#_bt_load-deduplicate](../../../raw/postgres-17/src/backend/access/nbtree/nbtsort.c#L1147-L1152), [nbtinsert.c#dedup-on-insert](../../../raw/postgres-17/src/backend/access/nbtree/nbtinsert.c#L2774-L2782) |
...
- **The reclaim test 11b should show is not pinned.** The fixture pins the condition - built off, switched on, no inserts - and the oracle measures whatever the rebuild returns, but how much a deduplicating rebuild of an unchanged file gives back is a function of the key's duplication and the posting-list packing, not of the switch. So 11b can prove that a method missed a large reclaimable fraction, and cannot state in advance what that fraction is.
...
- [btree_index.sql#deduplicate_items](../../../raw/postgres-17/src/test/regress/sql/btree_index.sql#L185-L223)
