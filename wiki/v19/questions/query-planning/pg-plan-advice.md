...
verified: false
verified_by_agent: claude-opus-5-max 2026-09-01T14:07:51Z
---
...
The feature's visible module landed in 28 commits that touched `contrib/pg_plan_advice/` (2026-03-12 through 2026-08-27), but the complete scoped source history also includes 20 core planner foundation/enabling/fix commits and test/doc support commits. Those are separated under [Source Commit History](#source-commit-history).
...
- DDL or relcache/syscache invalidation on a referenced relation forces a re-plan, at which point the current advice is re-applied [plancache.c#invalidation-overview](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L14-L42) [plancache.c#relation-invalidation](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L2146-L2184) [plancache.c#object-invalidation](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L2212-L2283).
- Role and database changes are a further re-plan point. As of `567286b762b` (2026-08-10, Nathan Bossart, CVE-2026-14666, back-patched through 14) the plan cache registers syscache callbacks on `pg_auth_members`, `pg_authid`, and `pg_database` [plancache.c#role-callbacks](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L164-L166). `PlanCacheRoleCallback()` then invalidates the role-dependent plans, so a role-membership, role-attribute, or database-ownership change is another point at which a cached plan is re-planned and the current advice re-read; changes to other databases' `pg_database` rows are ignored [plancache.c#PlanCacheRoleCallback](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L2321-L2369) [plancache.c#invalidation-overview](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L14-L42).
...
The 28 direct module commits are:
...
| `pgs_mask` helpers only clear bits (all eight writes are `&= ~`) | [pgpa_planner.c#L900-L933](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_planner.c#L900-L933) [pgpa_planner.c#L1137-L1149](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_planner.c#L1137-L1149) [pgpa_planner.c#L1845-L1858](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_planner.c#L1845-L1858) |
| The module never clears `PGS_CONSIDER_PARTITIONWISE`; partitionwise advice clears `PGS_JOIN_ANY` (induce) or `PGS_APPEND`/`PGS_MERGE_APPEND` (prevent) | [pgpa_planner.c#L916-L934](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_planner.c#L916-L934) [pgpa_planner.c#L1717-L1735](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_planner.c#L1717-L1735) [pathnodes.h#L52-L58](../../../../raw/postgres-19/src/include/nodes/pathnodes.h#L52-L58) |
| `joinrel_setup_hook` applies only the trove's `REL` slice (Gather + partitionwise) | [pgpa_planner.c#L740-L766](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_planner.c#L740-L766) [pgpa_trove.h#L33-L56](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_trove.h#L33-L56) |
...
| Custom-vs-generic re-plan policy and invalidation (when advice re-applies) | [plancache.c#L14-L42](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L14-L42) [plancache.c#L1183-L1231](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L1183-L1231) [plancache.c#L2146-L2184](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L2146-L2184) [plancache.c#L2212-L2283](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L2212-L2283) |
| Role/attribute/database-ownership changes also invalidate role-dependent cached plans (`567286b762b`) | [plancache.c#L164-L166](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L164-L166) [plancache.c#L2321-L2369](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c#L2321-L2369) |
...
| Feedback generation also needs `es != NULL` or one of two GUCs | [pgpa_planner.c#L253-L265](../../../../raw/postgres-19/contrib/pg_plan_advice/pgpa_planner.c#L253-L265) |
...
- The README's own "Future Work" lists four known gaps in the v19 feature: no control over the choice of aggregation (it would like to be able to "force sorted or grouped aggregation"), no knowledge of eager aggregation, no control over estimates as opposed to outcomes (unlike `pg_hint_plan`), and an uninvestigated interaction with GEQO [README#Future-Work](../../../../raw/postgres-19/contrib/pg_plan_advice/README#L250-L267). Sort order is a fifth gap, and it is named in `5883ff30`'s commit message rather than in the README: "decisions about how to perform aggregation are currently out of scope, as is choice of sort order". These are limitations of the shipped code, not of this page.
