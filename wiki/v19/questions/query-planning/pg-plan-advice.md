...
Naming fix, back-patched through 19. A simple `EXISTS` subplan can be converted to a hashed `ANY` subplan as an alternative implementation; `make_subplan()` used to name that converted planning root from the `EXISTS` sublink string, so both alternatives carried `exists_` names. It now passes the literal prefix `"exists_to_any"` to `choose_plan_name()`, so EXPLAIN output and `pg_plan_advice` target names distinguish the converted alternative from the original [subselect.c#make_subplan-exists_to_any](../../../../raw/postgres-19/src/backend/optimizer/plan/subselect.c#L271-L280). Planner behavior is unchanged: only the user-visible name changes. The module's `alternatives` test now shows `SubPlan exists_to_any_1` and advises `DO_NOT_SCAN(alt_t2@exists_to_any_1)` where it previously said `exists_2` [alternatives.out#exists_to_any](../../../../raw/postgres-19/contrib/pg_plan_advice/expected/alternatives.out#L15-L24) [alternatives.sql:25](../../../../raw/postgres-19/contrib/pg_plan_advice/sql/alternatives.sql#L25). Its two follow-ups — `3d2f2eb1664` (pgindent for the `subselect.c` change) and `c9bd90242db` (`.git-blame-ignore-revs`) — touch no module file and are not counted here, and the 20 core planner commits are unaffected.
...
- [plancache.c](../../../../raw/postgres-19/src/backend/utils/cache/plancache.c) — `choose_custom_plan` custom-vs-generic policy governing when a prepared statement re-plans (and thus re-reads advice), plus the relation/object and role/database invalidation callbacks that force such a re-plan.
