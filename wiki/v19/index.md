...
Behavioral claims cite the matching pinned checkout under `raw/postgres-19/`. This is the official PostgreSQL 19 beta 1 tag; there is no `REL_19_STABLE` branch yet. Filed coverage includes comprehensive walkthroughs of the new `pg_plan_advice` contrib module and the new in-core `REPACK` / `REPACK (CONCURRENTLY)` command, each with scoped source history pinned to `4b0bf078`.
