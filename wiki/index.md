...
- [Query Planner Statistics Sources in PostgreSQL 12 (unverified)](v12/questions/query-planner-statistics-sources.md) - Explains that the v12 planner does not use `pg_stat_all_tables`; it uses `pg_class` relation-size stats, `pg_statistic` column stats, extended statistics catalogs, index/FK/constraint metadata, and explicit planner statistics hooks.
