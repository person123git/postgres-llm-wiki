...
- [Planner Penalties for Bloated Indexes in PostgreSQL 12 (unverified)](v12/questions/bloated-indexes-query-planner.md) - Explains how v12 penalizes B-tree bloat indirectly through physical index pages, B-tree height, cache/page-cost modeling, and executor leaf-page walking; expands how `RelationGetNumberOfBlocks()` obtains `index->pages` through storage-manager file-size probes rather than index-page scans; adds a proposed `avg_leaf_density` / `leaf_fragmentation` triage heuristic for REINDEX candidates.
